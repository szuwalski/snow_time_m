...
mod_names <- seq(2019,2011)
.MODELDIR = c("./20_sq_wts/retro/2019/","./20_sq_wts/retro/2018/","./20_sq_wts/retro/2017/",
              "./20_sq_wts/retro/2016/","./20_sq_wts/retro/2015/",
              "./20_sq_wts/retro/2014/","./20_sq_wts/retro/2013/",
              "./20_sq_wts/retro/2012/","./20_sq_wts/retro/2011/")
...
plot(M[[1]]$obs_cpue~M[[1]]$mod_yrs,type='l',ylim=c(0,max(M[[5]]$ssb)),las=1,lwd=2,ylab="MMB (1,000t)",xlab="Year")
for(x in 2:length(mod_names))
  lines(M[[x]]$ssb~M[[x]]$mod_yrs,lty=1,col=x,lwd=2)
...
colnames(cpue_df)<-c("q","year","season","fleet","sex","mature","obs","cv","uh","pred",'peel')
...
p<-ggplot(data=temp) +
  ...
  geom_line(aes(x=year,y=pred,group=peel,color=peel),lwd=1) +
...
for(x in 1:length(ref_ssb))
  peel_ssb[x]<-filter(temp,year==max_yr-x,peel==x+1)$pred
...
#================================================
# 
#Scenarios   <-c("20_sq","20_sq_wts","20_sq_wts_q_vary","20_sq_wts_m_vary","20_sq_wts_both_vary","20_sq_wts_vast")
png("plots/mult_retro.png",height=8,width=4,res=400,units='in')
retro_inds<-c(1,2,3,4)
retro_yrs<-seq(2019,2011)
retro<-length(retro_yrs)
par(mfrow=c(length(retro_inds),1),mar=c(.1,.1,.1,.1),oma=c(4,4,1,1))
for(x in retro_inds)
{
#==plot the observed MMB
plot(as.numeric(unlist(snowad.rep[[x]]$"Observed survey male spawning biomass"))[1:SurvYrN[[x]]]~na.omit(SurveyYrs[[x]]),
...
for(j in 1:length(SurveyYrs[[2]]))
{
  segments(x0=SurveyYrs[[2]][j],x1=na.omit(SurveyYrs[[2]])[j],
           ...
           y1=as.numeric(unlist(snowad.rep[[2]]$"Observed survey male spawning biomass"))[j] * exp(1.96*sqrt(log(1+snowad.rep[[2]]$"survey CV"[2,]^2)))[j],col='grey')
}
if(x==retro_inds[length(retro_inds)]) axis(side=1)
#==plot the retrospective
tempbio<-matrix(nrow=retro+1,ncol=SurvYrN[[x]])
tempbio[1,]<-as.numeric(unlist(snowad.rep[[x]]$"Predicted Male survey mature Biomass"))[1:SurvYrN[[x]]]
for(y in 1:retro)
{  
  infile<-paste(Scenarios[x],"/retro/",retro_yrs[y],"/R_input.txt",sep="")
  temp <-readList(infile) 
  tempbio[(y+1),1:(ncol(tempbio)-(y))]<-as.numeric(unlist(temp$"Predicted Male survey mature Biomass"))[1:(SurvYrN[[x]]-y)]

    lines(as.numeric(unlist(temp$"Predicted Male survey mature Biomass"))[1:(SurvYrN[[x]]-y)] ~ na.omit(SurveyYrs[[x]][1:(SurvYrN[[x]]-y)]),type="l",
          lty=1,lwd=1,col=x)
}
...
for(y in 1:9)
{
  retro_pat[y]<- (  tempbio[(y+1),ncol(tempbio)-y] -tempbio[1,ncol(tempbio)-y] ) / tempbio[1,ncol(tempbio)-y]
}
