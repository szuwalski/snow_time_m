 ...
 if(j<=nsellen_srv1)
   {
    ...
    // else
    // { 
    // sel_srv3(2,j)=srv3_q(iy)*1./(1.+mfexp(-1.*log(19.)*(length_bins(j)-srv3_sel50)/(srv3_sel95-srv3_sel50)));
    // sel_srv3(1,j)=srv3_q_f(iy)*1./(1.+mfexp(-1.*log(19.)*(length_bins(j)-srv3_sel50_f)/(srv3_sel95_f-srv3_sel50_f)));
    // }
  ...
  if(active(srv3_q))
   {
	...
	srv_q_like += srv_q_smooth_wt *norm2(first_difference(first_difference(srv3_q_f)));
    f+=srv_q_like;
    Fout(31)=srv_q_like;
    }
