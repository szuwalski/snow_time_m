...
cout<<nobs_growm<<endl;  		 
 ad_comm::change_datafile_name("2016sc.ctl");
  ...
  srv_q_smooth_wt.allocate("srv_q_smooth_wt");
  ...
  mdev_smooth.allocate("mdev_smooth");
 ...
 model_data(argc,argv) , function_minimizer(sz)
{
  ...
  deltaf.allocate(5.0,150.0,growth_phase2,"deltaf");
  ...
  srv3_q.allocate(1988,endyr,0.20,1.0000,survsel1_phase,"srv3_q");
  srv3_sel95.allocate(40.0,200.0,survsel1_phase,"srv3_sel95");
  srv3_sel50.allocate(25.0,90.0,survsel1_phase,"srv3_sel50");
  srv3_q_f.allocate(1988,endyr,0.20,1.0000,survsel1_phase,"srv3_q_f");
  ...
  m_dev_m.allocate(styr,endyr,-3,3,natM_phase,"m_dev_m");
  m_dev_f.allocate(styr,endyr,-3,3,natM_phase,"m_dev_f");
  ...
  sel_srv2.allocate(1,2,1,nlenm,"sel_srv2");
    ...
    sel_srv2.initialize();
  #endif
  sel_srv3.allocate(1,2,1988,endyr+Nproj,1,nlenm,"sel_srv3");
  #ifndef NO_AD_INITIALIZE
    sel_srv3.initialize();
  #endif
  ...
  #endif
  like_mdev.allocate("like_mdev");
  #ifndef NO_AD_INITIALIZE
  like_mdev.initialize();
  #endif
  srv_q_like.allocate("srv_q_like");
  #ifndef NO_AD_INITIALIZE
  srv_q_like.initialize();
  #endif
  ...
  call_no.initialize();
  #endif
  M_matn_mu.allocate(1,2,"M_matn_mu");
  #ifndef NO_AD_INITIALIZE
    M_matn_mu.initialize();
  #endif
  M_mato_mu.allocate(1,2,"M_mato_mu");
  #ifndef NO_AD_INITIALIZE
    M_mato_mu.initialize();
  #endif
  M_matn.allocate(1,2,styr,endyr+Nproj,"M_matn");
  #ifndef NO_AD_INITIALIZE
    M_matn.initialize();
  #endif
  M_mato.allocate(1,2,styr,endyr+Nproj,"M_mato");
  #ifndef NO_AD_INITIALIZE
    M_mato.initialize();
  #endif
  M.allocate(1,2,"M");
...
void model_parameters::userfunction(void)
{
 ...
 dvariable fut_m_m, fut_m_f;
    ...
    M_matn_mu(2)=M_matn_in(2)*Mmult;  //natural mortality mature new shell female/male
    M_mato_mu(2)=M_mato_in(2)*Mmult;  //natural mortality mature old shell female/male
    M_matn_mu(1)=M_matn_in(1)*Mmultf;  //natural mortality mature new shell female/male
    M_mato_mu(1)=M_mato_in(1)*Mmultf;  //natural mortality mature old shell female/male
	for(j=styr;j<=endyr;j++)
	 {	
      M_matn(2,j) = exp(log(M_matn_mu(2)) + m_dev_m(j));
      M_mato(2,j) = exp(log(M_mato_mu(2)) + m_dev_m(j));
      M_matn(1,j) = exp(log(M_matn_mu(1)) + m_dev_f(j));
      M_mato(1,j) = exp(log(M_mato_mu(1)) + m_dev_f(j));
	 }
	fut_m_m.initialize();
	fut_m_f.initialize();
	for(j=endyr-4;j<=endyr;j++)
	{
 	 fut_m_m += M_matn(2,j);
	 fut_m_f += M_matn(1,j);
	}
...
void model_parameters::get_selectivity(void)
{
 ...
 for (j=1;j<=nlenm;j++)
  { 
   ...
   for (j=1;j<=nlenm;j++)
    { 
    sel_srv3(2,iy,j)=srv3_q(iy)*1./(1.+mfexp(-1.*log(19.)*(length_bins(j)-srv3_sel50)/(srv3_sel95-srv3_sel50)));
   //female survey 3
   sel_srv3(1,iy,j)=srv3_q_f(iy)*1./(1.+mfexp(-1.*log(19.)*(length_bins(j)-srv3_sel50_f)/(srv3_sel95_f-srv3_sel50_f)));
    }
   }
   if(iy>endyr)
   {
	 sel_srv3(1,iy) = sel_srv3(1,endyr);
	 sel_srv3(2,iy) = sel_srv3(2,endyr);
   }
  }  //end of year loop
 ...
 //female survey 3
   ...
   sel_srvnmfs(1,j)= sel_srvind(1,j)*sel_srv3(1,2009,j);
   sel_srvnmfs(2,j)= sel_srvind(2,j)*sel_srv3(2,2009,j);
   ...
   sel_srv10nmfs(1,j)=sel_srv10ind(1,j)*sel_srv3(1,2010,j);
   sel_srv10nmfs(2,j)=sel_srv10ind(2,j)*sel_srv3(2,2010,j); 
...
void model_parameters::get_num_at_len_yr(void)
{
 ...
 for(k=1;k<=2;k++)
  {
      ...
      dvar_vector tmpm 			= elem_prod(moltp_mat(k)*mfexp(-(1-catch_midpt(i))*M_matn(k,i)), elem_prod(Smat(k,1,i),mfexp(-catch_midpt(i)*M_matn(k,i))*natlength_mnew(k,i)));
      natlength_mnew(k,i+1)		=   tmpm * len_len(k);
      dvar_vector tmpmo			= elem_prod(moltp_mat(k)*mfexp(-(1-catch_midpt(i))*M_mato(k,i)), elem_prod(Smat(k,2,i),mfexp(-catch_midpt(i)*M_mato(k,i))*natlength_mold(k,i)));
      natlength_mnew(k,i+1) 	+=   tmpmo * len_len(k);
      natlength_mold(k,i+1)		= (mfexp(-(1-catch_midpt(i))*M_matn(k,i)) * elem_prod(Smat(k,1,i),mfexp(-catch_midpt(i)*M_matn(k,i))*natlength_mnew(k,i))) +  (mfexp(-(1-catch_midpt(i))*M_mato(k,i)) * elem_prod(Smat(k,2,i),mfexp(-catch_midpt(i)*M_mato(k,i))*natlength_mold(k,i))) - tmpm-tmpmo;
 ...
 if(i<=endyr)
      ...
      natl_mnew_fishtime(k,i)	=  mfexp(-catch_midpt(i)*M_matn(k,i))*natlength_mnew(k,i);
      natl_mold_fishtime(k,i) 	=  mfexp(-catch_midpt(i)*M_mato(k,i))*natlength_mold(k,i);
      ...
      mspbio_matetime(i) 		= (elem_prod(Smat(2,1,i)*mfexp(-(spmo)*M_matn(2,i)),mfexp(-catch_midpt(i)*M_matn(2,i))*natlength_mnew(2,i))+elem_prod(Smat(2,2,i)*mfexp(-(spmo)*M_mato(2,i)),mfexp(-catch_midpt(i)*M_mato(2,i))*natlength_mold(2,i)))*wtm;
      fspbio_matetime(i) 		= (elem_prod(Smat(1,1,i)*mfexp(-(spmo)*M_matn(1,i)),mfexp(-catch_midpt(i)*M_matn(1,i))*natlength_mnew(1,i))+elem_prod(Smat(1,2,i)*mfexp(-(spmo)*M_mato(1,i)),mfexp(-catch_midpt(i)*M_mato(1,i))*natlength_mold(1,i)))*wtf(2);
    ...
    if(k>1)
	{
     for(j=1;j<=nlenm;j++)
       {
        emspnum_old_matetime(i) 		+= Smat(2,2,i,j)*mfexp(-(spmo)*M_mato(2,i))*mfexp(-catch_midpt(i)*M_mato(2,i))*natlength_mold(2,i,j);
        mspnum_matetime(i) 				+= Smat(2,1,i,j)*mfexp(-(spmo)*M_matn(2,i))*mfexp(-catch_midpt(i)*M_matn(2,i))*natlength_mnew(2,i,j) + Smat(2,2,i,j)*mfexp(-(spmo)*M_mato(2,i))*mfexp(-catch_midpt(i)*M_mato(2,i))*natlength_mold(2,i,j);
        mspbio_old_matetime(i) 			+= (Smat(2,2,i,j)*mfexp(-(spmo)*M_mato(2,i))*mfexp(-catch_midpt(i)*M_mato(2,i))*natlength_mold(2,i,j))*wtm(j);
        fspnum_new_matetime(i) 			+= (Smat(1,1,i,j)*mfexp(-(spmo)*M_matn(1,i))*mfexp(-catch_midpt(i)*M_matn(1,i))*natlength_mnew(1,i,j));
        fspbio_new_matetime(i)			+= (Smat(1,1,i,j)*mfexp(-(spmo)*M_matn(1,i))*mfexp(-catch_midpt(i)*M_matn(1,i))*natlength_mnew(1,i,j))*wtf(2,j);
        efspnum_matetime(i) 			+= (Smat(1,1,i,j)*mfexp(-(spmo)*M_matn(1,i))*mfexp(-catch_midpt(i)*M_matn(1,i))*natlength_mnew(1,i,j)+Smat(1,2,i,j)*mfexp(-(spmo)*M_mato(1,i))*mfexp(-catch_midpt(i)*M_mato(1,i))*natlength_mold(1,i,j));
        if(j>15){
 ...
 if(i ==endyr)
  ...
  natl_mnew_fishtime(1,endyr) = (mfexp(-catch_midpt(endyr)*M_matn(1,i))*natlength_mnew(1,endyr));
  natl_mold_fishtime(1,endyr) = (mfexp(-catch_midpt(endyr)*M_mato(1,i))*natlength_mold(1,endyr));
  natl_mnew_fishtime(2,endyr) = (mfexp(-catch_midpt(endyr)*M_matn(2,i))*natlength_mnew(2,endyr));
  natl_mold_fishtime(2,endyr) = (mfexp(-catch_midpt(endyr)*M_mato(2,i))*natlength_mold(2,endyr));
  ...
  mspbio_matetime(endyr) = (elem_prod(Smat(2,1,endyr)*mfexp(-(spmo)*M_matn(2,i)),mfexp(-catch_midpt(endyr)*M_matn(2,i))*natlength_mnew(2,endyr))+elem_prod(Smat(2,2,endyr)*mfexp(-(spmo)*M_mato(2,i)),mfexp(-catch_midpt(endyr)*M_mato(2,i))*natlength_mold(2,endyr)))*wtm;
  fspbio_matetime(endyr) = (elem_prod(Smat(1,1,endyr)*mfexp(-(spmo)*M_matn(1,i)),mfexp(-catch_midpt(endyr)*M_matn(1,i))*natlength_mnew(1,endyr))+elem_prod(Smat(1,2,endyr)*mfexp(-(spmo)*M_mato(1,i)),mfexp(-catch_midpt(endyr)*M_mato(1,i))*natlength_mold(1,endyr)))*wtf(2);
  ...
  for(j=1;j<=nlenm;j++)
   {
      emspnum_old_matetime(endyr) 	+= Smat(2,2,endyr,j)*mfexp(-(spmo)*M_mato(2,i))*mfexp(-catch_midpt(endyr)*M_mato(2,i))*natlength_mold(2,endyr,j);
      mspnum_matetime(endyr) 		+= Smat(2,1,endyr,j)*mfexp(-(spmo)*M_matn(2,i))*mfexp(-catch_midpt(endyr)*M_matn(2,i))*natlength_mnew(2,endyr,j) + Smat(2,2,endyr,j)*mfexp(-(spmo)*M_mato(2,i))*mfexp(-catch_midpt(endyr)*M_mato(2,i))*natlength_mold(2,endyr,j);
        mspbio_old_matetime(endyr) 	+= (Smat(2,2,endyr,j)*mfexp(-(spmo)*M_mato(2,i))*mfexp(-catch_midpt(endyr)*M_mato(2,i))*natlength_mold(2,endyr,j))*wtm(j);
        fspnum_new_matetime(endyr) 	+= (Smat(1,1,endyr,j)*mfexp(-(spmo)*M_matn(1,i))*mfexp(-catch_midpt(endyr)*M_matn(1,i))*natlength_mnew(1,endyr,j));
        fspbio_new_matetime(endyr)	+= (Smat(1,1,endyr,j)*mfexp(-(spmo)*M_matn(1,i))*mfexp(-catch_midpt(endyr)*M_matn(1,i))*natlength_mnew(1,endyr,j))*wtf(2,j);
        efspnum_matetime(endyr) 	+= (Smat(1,1,endyr,j)*mfexp(-(spmo)*M_matn(1,i))*mfexp(-catch_midpt(endyr)*M_matn(1,i))*natlength_mnew(1,endyr,j)+Smat(1,2,endyr,j)*mfexp(-(spmo)*M_mato(1,i))*mfexp(-catch_midpt(endyr)*M_mato(1,i))*natlength_mold(1,endyr,j));
        if(j>15){
  ...
  for(k=1;k<=2;k++)
      {
       ...
       if(i>1987){
        totn_srv1(k,i)=(natlength(k,i)*sel_srv3(k,i));
        if(i==2009){
    ...
    if(i>1987)
	{
     fspbio_srv1(i) = q1*natlength_mat(1,i)*elem_prod(wtf(2),sel_srv3(1,i));
     mspbio_srv1(i) = q1*natlength_mat(2,i)*elem_prod(wtm,sel_srv3(2,i));
     fspbio_srv1_num(1,i) = q1*natlength_mnew(1,i)*sel_srv3(1,i);
     mspbio_srv1_num(1,i) = q1*natlength_mnew(2,i)*sel_srv3(2,i);
     fspbio_srv1_num(2,i) = q1*natlength_mold(1,i)*sel_srv3(1,i);
     mspbio_srv1_num(2,i) = q1*natlength_mold(2,i)*sel_srv3(2,i);
      if(i==2009)
        {
 ...
 for(k=1;k<=2;k++)
  ...
  if(i>1981 && i<1988)
  {
   ...
   if(i>1987)
   {
      pred_srv1(k,i) = q1*elem_prod(natlength(k,i),sel_srv3(k,i));
     if(k<2)
	 {
      pred_srv1_bioms(k,i) = q1*((natlength_inew(k,i)*elem_prod(sel_srv3(k,i),wtf(1)))+((natlength_mnew(k,i)+natlength_mold(k,i))*elem_prod(sel_srv3(k,i),wtf(2))));
      }
     else
	 {
      pred_srv1_bioms(k,i) = q1*(natlength(k,i)*elem_prod(sel_srv3(k,i),wtm));
      }
      pred_p_srv1_len_new(1,k,i)=elem_prod(sel_srv3(k,i),natlength_inew(k,i))/(totn_srv1(1,i)+totn_srv1(2,i));
      pred_p_srv1_len_old(1,k,i)=elem_prod(sel_srv3(k,i),natlength_iold(k,i))/(totn_srv1(1,i)+totn_srv1(2,i));
      pred_p_srv1_len_new(2,k,i)=elem_prod(sel_srv3(k,i),natlength_mnew(k,i))/(totn_srv1(1,i)+totn_srv1(2,i));
      pred_p_srv1_len_old(2,k,i)=elem_prod(sel_srv3(k,i),natlength_mold(k,i))/(totn_srv1(1,i)+totn_srv1(2,i));
    }
  ...
  mspbios=mspbio_matetime;
    ...
    legal_srv_males_o(i)=0.0;
        ...
        if(i>1987)
		{
         legal_srv_males(i)=0.5*natlength(2,i,16)*sel_srv3(2,i,16);
         legal_srv_males_n(i)=0.5*natlength_new(2,i,16)*sel_srv3(2,i,16);
         legal_srv_males_o(i)=0.5*natlength_old(2,i,16)*sel_srv3(2,i,16);
        }
	...
	for(j=17;j<=nlenm;j++)
    {
        ...
        if(i>1987)
		{
          legal_srv_males(i)+=natlength(2,i,j)*sel_srv3(2,i,j);
          legal_srv_males_n(i)+=natlength_new(2,i,j)*sel_srv3(2,i,j);
          legal_srv_males_o(i)+=natlength_old(2,i,j)*sel_srv3(2,i,j);
          legal_srv_males_bio(i)+=natlength(2,i,j)*sel_srv3(2,i,j)*wtm(j);
          }
    }
...
void model_parameters::evaluate_the_objective_function(void)
{
 ...
 for(mat=1;mat<=2;mat++)
  ...
  if(active(m_dev_m))
   {
    like_mdev = mdev_smooth *norm2(first_difference(first_difference(m_dev_m)));
	like_mdev += mdev_smooth *norm2(first_difference(first_difference(m_dev_f)));
    f+=like_mdev;
    Fout(31)=like_mdev;
    }
  if(active(srv3_q))
   {
    srv_q_like = srv_q_smooth_wt *norm2(first_difference(first_difference(srv3_q)));
	srv_q_like += srv_q_smooth_wt *norm2(first_difference(first_difference(srv3_q)));
    f+=srv_q_like;
    Fout(32)=srv_q_like;
    }
...
void model_parameters::report(const dvector& gradients)
{
 adstring ad_tmp=initial_params::get_reportfile_name();
  ...
  for(i=1988;i<=endyr;i++)
	  report << sel_srv3(2,i) << endl;
    if (last_phase())
  {
 ...
 // R_out <<elem_div(pred_catch_no_gt101(styr,endyr-1),obs_tmp(styr,endyr-1)*mfexp(-M_matn(2,styr,endyr-1)*(7/12))) << endl;
     for(i=styr;i<endyr;i++)
        {
 ...
 // R_out <<elem_div(pred_catch(styr,endyr-1),((obs_srv1_spbiom(2))(styr,endyr-1))*mfexp(-M_matn(2,styr,endyr-1)*(7/12))) << endl;
